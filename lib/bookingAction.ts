...
export function htmlPage(title: string, heading: string, body: string, color: string) {
  return new NextResponse(
    `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <title>${title}</title>
    <style>
      body{font-family:sans-serif;background:#0d0906;color:#fff;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;}
      ...
      p{color:#ccc;line-height:1.6;margin:0 0 8px;}
    </style>
  ...
  try {
    ...
    if (booking.status !== 'pending') {
      ...
      return htmlPage('Already Actioned', label,
        `<p>This booking was already <strong>${booking.status}</strong>.</p>
         <p>Guest: ${booking.guest_name}<br>${booking.check_in} → ${booking.check_out}</p>`,
...
// ─── Data-returning variant used by server page components ──────────────────

type ActionResult =
  | { ok: false; error: string }
  | { ok: true; alreadyActioned: true; status: string; guestName: string; checkIn: string; checkOut: string }
  | { ok: true; alreadyActioned: false; guestName: string; unitName: string; checkIn: string; checkOut: string; nights: number; lockCode: string; notified: string };

export async function runBookingAction(id: string, action: 'confirm' | 'deny'): Promise<ActionResult> {
  try {
    const { getSql } = await import('@/lib/db');
    const sql = getSql();
    ...
    if (booking.status !== 'pending') {
      return { ok: true, alreadyActioned: true, status: booking.status, guestName: booking.guest_name, checkIn: booking.check_in, checkOut: booking.check_out };
    }
    ...
    if (action === 'confirm') {
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
              ...
              <div style="background:#f0f7f0;border-left:4px solid #4a7043;padding:16px;margin:20px 0;border-radius:8px;">
                <p style="margin:0 0 6px;font-size:14px;color:#4a7043;font-weight:bold;">🏠 Guest Portal</p>
                <p style="margin:0 0 8px;font-size:13px;color:#555;">Access your booking details at <a href="${PORTAL_URL}" style="color:#4a7043;font-weight:bold;">${PORTAL_URL}</a></p>
                <p style="margin:0;font-size:13px;color:#555;">Log in with ${loginHint}.</p>
              </div>
              <div style="background:#fff8f0;border-left:4px solid #c36a4f;padding:16px;margin:20px 0;border-radius:8px;">
                <p style="margin:0 0 6px;font-size:14px;color:#c36a4f;font-weight:bold;">✈️ Flying In? Request a Ride</p>
                <p style="margin:0;font-size:13px;color:#555;">Use the <strong>"Request a Ride"</strong> feature in your Guest Portal to arrange an airport pickup.</p>
              </div>
              <p style="color:#888;font-size:13px;margin-top:32px;">Questions? Reply to this email or message us on WhatsApp.<br>— La Casa Oaxaca</p>
            </div>
          `,
      ...
      if (hasPhone) {
        const loginHint = hasEmail ? 'code, email, or phone' : 'code or phone';
        const sms = `La Casa Oaxaca: Stay confirmed! Check-in: ${booking.check_in}. Lock code: ${booking.lock_code}. Portal: ${PORTAL_URL} (log in with your ${loginHint}). Flying in? Request airport pickup in the portal!`;
        await sendSms(booking.phone, sms).catch((err: unknown) => console.error('[SMS]', err));
      }
    } else {
      if (hasEmail && process.env.RESEND_API_KEY) {
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto;">
              <h2 style="color:#8d4a3f;">Update on Your Booking Request</h2>
              <p>Hi ${firstName},</p>
              <p>Unfortunately, we're unable to accommodate your booking request for <strong>${booking.unit_name}</strong> from <strong>${booking.check_in}</strong> to <strong>${booking.check_out}</strong>. This may be due to a scheduling conflict.</p>
              <div style="background:#fff8f0;border-left:4px solid #c36a4f;padding:16px;margin:20px 0;border-radius:8px;">
                <p style="margin:0;font-size:13px;color:#555;">💳 <strong>Escrow Released:</strong> Any payment held will be returned to your original payment method within <strong>3–5 business days</strong>.</p>
              </div>
              <p style="color:#888;font-size:13px;margin-top:32px;">Questions? Reply to this email or message us on WhatsApp.<br>— La Casa Oaxaca</p>
            </div>
          `,
    ...
    const notified = [hasEmail && 'email', hasPhone && 'SMS'].filter(Boolean).join(' and ') || 'no contact method on file';
    return { ok: true, alreadyActioned: false, guestName: booking.guest_name, unitName: booking.unit_name, checkIn: booking.check_in, checkOut: booking.check_out, nights: booking.nights, lockCode: booking.lock_code, notified };
  } catch (err) {
    console.error('[runBookingAction]', err);
